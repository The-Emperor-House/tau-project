...
export default function DashboardNews() {
  ...
  const onToggleFeatured = async (item) => {
    try {
      const { getSession } = await import("next-auth/react");
      const session = await getSession();
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/news/${item.id}/feature`, {
        method: "PATCH",
        headers: {
      ...
      toast.success(item.isFeatured ? "Removed from Home" : "Set as Featured on Home");
      fetchNews();
    } catch {
        ...
        {(loading ? skeletons : news).map((it, i) => (
          <div key={it?.id ?? i} className="rounded-xl overflow-hidden bg-neutral-900 border border-neutral-800 flex flex-col">
            <div className="relative aspect-video bg-neutral-800">
              ...
              {!loading && it.isFeatured && (
                <span className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-black bg-[#cc8f2a] rounded-full">
                ...
                <>
                  ...
                  <div className="flex gap-2 mt-auto pt-4">
                    <button
                      ...
                      title={it.isFeatured ? "Remove from Home" : "Show on Home"}
                      className={`flex items-center gap-1.5 px-3 py-2 min-h-[40px] text-xs rounded-lg border transition-colors ${
                        it.isFeatured
                          ? "text-[#cc8f2a] border-[#cc8f2a]/40 bg-[#cc8f2a]/10"
                          : "text-neutral-400 border-neutral-700 hover:bg-neutral-800"
                      }`}
                    >
                      <StarIcon filled={it.isFeatured} />
                    </button>
                    <button
                      onClick={() => onEdit(it)}
...
function StarIcon({ filled }) { return <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>; }
